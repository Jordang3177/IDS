...
def generating_tree(self, current_depth):
    instances = 0
    count = 0
    ...
    for j in range(0, len(self.name)):
        ...
        if self.name[j] != "b" and instances == 0:
            ...
            if count == sides - 1:
                count = 0
                new_name = self.name[:j + 1] + self.name[j + sides:]
                new_name = Tree(new_name)
                new_name = generating_tree(new_name, current_depth + 1)
                self.add_child(new_name)
            else:
                count = 0
                new_name = self.name[:j + 1] + "b" + self.name[j + 1:]
                new_name = Tree(new_name)
                new_name = generating_tree(new_name, current_depth + 1)
                self.add_child(new_name)
        elif self.name[j] != "b" and instances != 0:
            subname = self.name[j + 1:]
            for k in range(0, len(subname)):
                if subname[k] != "b":
                    break
